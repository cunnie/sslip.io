	...
	Describe("for more complex assertions", func() {
		...
		When("ns.nip.io is queried", func() {
			It("returns all the A records", func() {
				...
				Eventually(digSession, 1).Should(Exit(0))
				Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeA ns.nip.io. \? 167.172.4.236, 104.155.144.4, 5.78.28.211, 51.75.53.19\n`))
			})
			It("returns all the AAAA records", func() {
				...
				Eventually(digSession, 1).Should(Exit(0))
				Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeAAAA ns.nip.io. \? 2400:6180:0:d2:0:2:e3e7:0, 2600:1900:4000:4d12::, 2a01:4ff:1f2:10d::, 2001:41d0:602:2313::1\n`))
			})
		})
		When("ns.sslip.io is queried", func() {
			It("returns all the A records", func() {
				...
				Eventually(digSession).Should(Say(`51.75.53.19`))
				Eventually(digSession, 1).Should(Exit(0))
				Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeA ns.sslip.io. \? 167.172.4.236, 104.155.144.4, 5.78.28.211, 51.75.53.19\n`))
			})
			It("returns all the AAAA records", func() {
				...
				Eventually(digSession).Should(Say(`2001:41d0:602:2313::1`))
				Eventually(digSession, 1).Should(Exit(0))
				Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeAAAA ns.sslip.io. \? 2400:6180:0:d2:0:2:e3e7:0, 2600:1900:4000:4d12::, 2a01:4ff:1f2:10d::, 2001:41d0:602:2313::1\n`))
			})
		})
		...
		When("there are multiple NS records returned (e.g. almost any NS query)", func() {
			It("returns all the records", func() {
				...
				Eventually(digSession).Should(Say(`ns-do-sg.sslip.io..*167.172.4.236\n`))
				Eventually(digSession).Should(Say(`ns-do-sg.sslip.io..*2400:6180:0:d2:0:2:e3e7:0\n`))
				Eventually(digSession).Should(Say(`ns-hetzner.sslip.io..*5.78.28.211\n`))
				Eventually(digSession).Should(Say(`ns-hetzner.sslip.io..*2a01:4ff:1f2:10d::\n`))
				...
				Eventually(string(serverSession.Err.Contents())).Should(MatchRegexp(`TypeNS example.com. \? ns-do-sg.sslip.io., ns-hetzner.sslip.io., ns-ovh.sslip.io.\n`))
			})
		})
	...
	Describe(`The domain blocklist`, func() {
		DescribeTable("when queried",
			...
			Entry("an NS record with acme_challenge with a forbidden string is not delegated",
				...
				`\Ans-[a-z-]+.sslip.io.\nns-[a-z-]+.sslip.io.\nns-[a-z-]+.sslip.io.\n\z`,
				`TypeNS _acme-challenge.raiffeisen.fe80--.sslip.io. \? ns-do-sg.sslip.io., ns-hetzner.sslip.io., ns-ovh.sslip.io.\n$`),
